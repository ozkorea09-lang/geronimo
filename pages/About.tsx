...
const About: React.FC = () => {
  ...
  if (!data) return <div className="min-h-screen bg-brand-latte text-brand-text pt-20 flex items-center justify-center font-serif text-xl">Loading...</div>;
    ...
    <div className="w-full bg-brand-latte text-brand-text pt-20">
          ...
          <div className="md:w-1/2">
            <div className="relative">
              <img 
                ...
                alt="Cafe Interior" 
                className="w-full rounded-lg shadow-wood transition-all duration-700 hover:shadow-2xl border-2 border-brand-wood/20"
              />
              <div className="absolute -bottom-12 -right-12 w-2/3 hidden md:block">
                 <img 
                  src={data.story.imageSub} 
                  alt="Coffee Detail" 
                  className="w-full rounded-lg shadow-wood border-8 border-brand-latte transition-all duration-700 hover:scale-105"
                />
              </div>
            </div>
          </div>
            ...
            {data.philosophy.items.map((item, idx) => (
              <div key={idx} className="p-12 bg-brand-latte border-2 border-brand-wood/10 hover:border-brand-gold transition-all duration-500 rounded-xl group shadow-wood hover:shadow-2xl hover:-translate-y-3">
                <div className="w-24 h-24 bg-brand-coffee/10 rounded-full flex items-center justify-center mx-auto mb-10 group-hover:bg-brand-gold transition-colors duration-500">
                  {idx === 0 && <Coffee className="text-brand-coffee w-12 h-12 group-hover:text-white transition-colors duration-500" />}
                  {idx === 1 && <Award className="text-brand-coffee w-12 h-12 group-hover:text-white transition-colors duration-500" />}
                  {idx === 2 && <Star className="text-brand-coffee w-12 h-12 group-hover:text-white transition-colors duration-500" />}
                </div>
                <h3 className="text-3xl font-serif text-brand-text mb-6 group-hover:text-brand-coffee transition-colors font-bold">{item.title}</h3>
      ...
      <section className="py-28">
        <div className="container mx-auto px-6 mb-16 flex flex-col md:flex-row justify-between items-end">
          ...
          <p className="text-brand-muted mt-6 md:mt-0 max-w-md text-right md:text-left text-lg font-medium">
          ...
          {data.gallery.images.map((img, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-brand-coffee/60 flex flex-col items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500 backdrop-blur-[2px]">
                <span className="text-brand-cream font-serif text-3xl transform translate-y-6 group-hover:translate-y-0 transition-transform duration-500 delay-75 border-b-2 border-brand-gold pb-2">
                  {img.caption}
                </span>
              </div>
            </div>
       ...
       {lightboxIndex !== null && data.gallery.images[lightboxIndex] && (
        ...
        >
          ...
          >
            ...
            />
            <div className="mt-8 text-center bg-brand-coffee/80 px-8 py-3 rounded-full border border-brand-gold/50 backdrop-blur-sm">
              <h3 className="text-brand-gold font-serif text-3xl">{data.gallery.images[lightboxIndex].caption}</h3>
            </div>
          </div>
